...
function game(iden,newboolean,stepback){
    if(newboolean){
        newgame(arr);
        isX = true;
        counter = 0;
        ...
        cellback.push(IdInt);
        counter++;
        document.getElementById("stepback").setAttribute("onclick","game(-1,false,true)");
        isX = !isX;
...
function newgame(arr1){
    removeClickStepBack();
    cellback = []
    for(var i=0;i<9;i++){
        if(arr1[i] === 'x' || arr1[i] === 'o'){
            document.getElementById(`img${i}`).remove();
            document.getElementById(`${i}`).setAttribute("onclick","game(id,false,false)");
        }
    }
}

function back(arr1,cell,count){
    if(count>0){
        document.getElementById(`img${cell}`).remove();
        document.getElementById(`${cell}`).setAttribute("onclick","game(id,false,false)");
        arr1[cell] = '';
        return arr1;
    } 
}
